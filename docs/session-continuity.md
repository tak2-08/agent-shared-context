...
## 세션 시작 프로토콜 (자동·무조건 — 수동 언급 불필요)

> 기억은 사용자가 언급해야 가져오는 것이 아니다. 새 세션은 첫 실질 요청 전에 먼저 DB를 읽고 이해하고 파악해 둔다. 아래 "3) 새 세션 복원"의 수동 절차와 같은 내용이지만, 트리거가 다르다 — 사용자가 "이어서"라고 말하지 않아도 **항상** 돈다.

- **트리거**: 새 세션의 첫 번째 실질 요청(질문·작업·분석 — 단순 인사 제외). 이 프로토콜을 건너뛰고 바로 본론에 들어가면 안 된다.
- **동작** (저토큰 순서대로):
  1. `Read agent-context/CURRENT.md` — 50 tok 진입점. 진행 중 작업·포인터를 파악한다.
  2. 진행 중 작업이 있으면 `node tools/agent-handoff.mjs load` — task/done/next (~280 tok).
  3. 요청에서 검색어 1~3개를 뽑아(프로젝트명·주제·고유명사·파일/경로·"전에·지난번·이어서" 같은 신호) `node tools/agent-search-lite.mjs "<query>"` 최소 1회 실행. 질문이 기억과 무관해 보여도 생략하지 않는다.
  4. 관련 적중 1~2개만 원문 Read. 미리보기·색인 요약만 보고 "없다"고 단정하지 않는다.
  5. 답변·작업 전에 회상 결과를 한 줄로 명시한다 — `기억 확인: <무엇을> — <출처 path>` 또는 `기억 확인: 관련 기록 없음`.
- **서브에이전트**에게도 동일 규칙이 상속됨을 전제로 한다. 위임 시 관련 쿼리나 기억 출처를 함께 넘긴다.
- 기억 도구(`tools/agent-memory.mjs`의 per-user 메모리)도 같은 프로토콜을 탄다 — 세션 시작 시 `search` → `get` 순으로 능동 회상하고, 수동 언급을 기다리지 않는다.
